...
Paper 向けのニックネーム・プレフィックス・2行目サブタイトル用プラグイン。チャット/表示名の整形、PlaceholderAPI、マルチ言語に対応。

## ハイライト
- ニックネーム/プレフィックスにカラーコード対応（旧式 `&` と RGB HEX `#RRGGBB`）、デフォルト色と一意性チェック付き。
- サブタイトル（2 行目）にカラーコード（旧式/HEX）と PlaceholderAPI 対応。
- マルチバイト文字（UTF-8）対応で、日本語などもそのまま扱えます。
